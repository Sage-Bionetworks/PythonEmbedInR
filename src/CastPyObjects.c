/*  ----------------------------------------------------------------------------
    ...
    Provides functions to converts non nested Python data types to R data types.
...
/*  ----------------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------------*/
SEXP test_sexps(void){
    Rprintf("INTSXP: %i\n", INTSXP);
    Rprintf("INTSXP Test: %i\n", INTSXP==13);
    Rprintf("VECSXP: %i\n", VECSXP);
    Rprintf("VECSXP Test: %i\n", VECSXP==13);
    return R_NilValue;
}
...
/*  ----------------------------------------------------------------------------
    ...
    py_obj_str 
      transforms  python objects into their string representaion

  ----------------------------------------------------------------------------*/
SEXP py_obj_str(PyObject *py_object){
    SEXP r_val;
    PyObject *py_str_representation;
    py_str_representation = PyObject_Str(py_object);
    r_val = py_to_r(py_str_representation, 0);
    Py_XDECREF(py_str_representation);
    return r_val;
}
...
SEXP py_dict_to_r_vec(PyObject *py_object, int r_vector_type){
    ...
    if (r_vector_type == 10){                                         // boolean
        for (long i=0; i < vec_len; i++){
            ...
            Py_XINCREF(item);
            SET_VECTOR_ELT(r_vec_names, i, py_to_r(item, 0));
            Py_XDECREF(item);
...
SEXP py_dict_to_r_list(PyObject *py_object, int simplify){
    ...
    for (long i=0; i < list_len; i++){
        ...
        item = PyList_GetItem(py_keys, PyLong_AsSsize_t(py_i));
        Py_XINCREF(item);
        SET_VECTOR_ELT(r_list_names, i, py_to_r(item, 0));
        Py_XDECREF(item);

        item = PyList_GetItem(py_values, PyLong_AsSsize_t(py_i));
        Py_XINCREF(item);
        SET_VECTOR_ELT(r_list, i, py_to_r(item, simplify));
        Py_XDECREF(item);
...
/*  ----------------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------------*/
SEXP py_to_r(PyObject *py_object, int simplify){
    SEXP r_val;
    int r_type;
    ...
    if ( PyNone_Check(py_object) ){                                         // None
    ...
    }else if( PyBool_Check(py_object) ){                              // Boolean
        int c_boolean;
        c_boolean = PY_TO_C_BOOLEAN(py_object);
        r_val =  c_to_r_boolean(c_boolean);

    }else if(PyInt_Check(py_object)){                                 // Integer
        long c_long;
        c_long = PY_TO_C_INTEGER(py_object);
        r_val = c_to_r_integer(c_long);

    }else if(PyLong_Check(py_object)){                                   // Long
        long c_long;
        c_long = PY_TO_C_LONG(py_object);
        r_val = c_to_r_integer(c_long);

    }else if(PyFloat_Check(py_object)){                                 // Float
        double c_double;
        c_double = PY_TO_C_DOUBLE(py_object);
        r_val = c_to_r_double(c_double);

    }else if(PyString_Check(py_object)){                               // String
        const char *c_string;
        c_string = PY_TO_C_STRING(py_object);
        r_val = c_to_r_string(c_string);

    }else if(PyUnicode_Check(py_object)){                             // Unicode
        const char *c_string;
        c_string = PY_TO_C_UNICODE(py_object);
        r_val = c_to_r_unicode(c_string); 

    }else if(PyTuple_Check(py_object)){                                 // Tuple
        if (simplify){
    ...
    }else if(PyList_Check(py_object)){                                   // List
        if (simplify){
        ...
        r_val = py_obj_str(py_object);
	    //<<<NOTE: This is unnecessary complicated I should make the 
        //         py_obj_string function in a way that it just returns C 
        //         string and use the macro on it!!>>>

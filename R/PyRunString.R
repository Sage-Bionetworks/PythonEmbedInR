...
#  -----------------------------------------------------------------------------
pyExecg <- function(code, returnValues=character(), returnToR=TRUE, 
...
", rv)
    ...
    mergeNamespaces <- as.integer(mergeNamespaces[1])
    override <- as.integer(override[1])
    returnToR <- as.integer(returnToR[1])
    
    if (getOption("winPython364")){
        ret_val <- try(.Call("py_run_string", code, mergeNamespaces, override, 
                             returnToR, simplify), silent = TRUE)
        msg <- makeErrorMsg()
        if (!is.null(msg)) stop(msg)
    }else{
        ret_val <- .Call("py_run_string", code, mergeNamespaces, override, 
                         returnToR, simplify)
    }
...
pyExecgIntern <- function(code, mergeNamespaces=FALSE,
                          override=TRUE, simplify=TRUE){
    if ( pyConnectionCheck() ) return(invisible(NULL))
    mergeNamespaces <- as.integer(mergeNamespaces[1])
    override <- as.integer(override[1])
    ret_val <- try(.Call("py_run_string", code, mergeNamespaces, override, TRUE, 
...
pyExecfile <- function(filename){
    ...
    pyExecp(sprintf("execfile('%s')", as.character(filename)[1]))
    if (getOption("winPython364")){
        msg <- makeErrorMsg()
        if (!is.null(msg)) stop(msg)
    }
...
pyPrint <- function(objName){
    if ( pyConnectionCheck() ) return(invisible(NULL))
    pyExec(sprintf("print(%s)", as.character(substitute(objName))))
    if (getOption("winPython364")){
        msg <- makeErrorMsg()
        if (!is.null(msg)) stop(msg)
    }

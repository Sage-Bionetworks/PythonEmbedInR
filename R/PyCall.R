...
#'         pyCall("sum", args=list(1:3), namespace="__builtin__")
#'     }else{
#'         pyCall("sum", args=list(1:3), namespace="builtins")
#'     }
#' }
#' pyExec('
#' def fun(**kwargs):
...
#  -----------------------------------------------------------
pyCall <- function(callableObj, args=NULL, kwargs=NULL, namespace='__main__', simplify=TRUE){
    if ( pyConnectionCheck() ) return(invisible(NULL))
    check_string(callableObj)
    splittedName <- unlist(strsplit(callableObj, ".", fixed=TRUE))
    
    if (getOption("winPython364")){
        returnValue <- try(.Call("py_call_object", namespace, splittedName, 
                         args, kwargs, simplify), silent=TRUE)
        msg <- makeErrorMsg()
        if (!is.null(msg)) stop(msg)
    }else{
        returnValue <- .Call("py_call_object", namespace, splittedName, 
                         args, kwargs, simplify)
    }
    returnValue
}
